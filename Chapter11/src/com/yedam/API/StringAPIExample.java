...
public class StringAPIExample {
	public static void main(String[] args) {
		...
		try {
			...
			//byte -> String
			String str8 = new String(byte2, "EUC-KR"); //위에서 바꿨기 때문에 똑같이 변경해야함 , ms949도 변경 가능함(소스가 포함되어 있는 것 같다? 되는 이유 추후 설명 추가)
			System.out.println(str8);			
		} catch (UnsupportedEncodingException e) {
			// 지원하지 않는 경우를 막기 위해 try/catch 를 컴파일러 오류 수정 힌트로 진행
			e.printStackTrace();
		}
		...
		int index = subject.indexOf("그래미"); //통채로 같은 단어가 있는 경우만 출력되고, -1이면 포함 안되어 있는거임
		...
		//1) 매개변수가 1개 일 때 -> 해당 위치로부터 뒤로 다 잘라서 가져옴
		String ssn3 = "123456-1234567";
		String firstNum = ssn3.substring(7);
		System.out.println(firstNum);
		
		//2) 매개변수가 2개 일 때 -> 시작위치 ~ 끝나는 위치 앞까지 잘라서 가져옴
		...
		String var = "또치";
		if(var.contains("또")) {
			System.out.println("또 포함 됨");  //한줄짜리 if문은 {}안 써도 되지만 웬만하면 가독성을 위해 쓰는 걸 추천
		}else {
